...
if exists("g:loaded_QFixGrep") && g:loaded_QFixGrep && !exists('g:fudist')
  finish
endif
let g:loaded_QFixGrep = 1
if v:version < 700
  finish
endif
...
function QFixGrepMenubar(menu, leader)
  ...
  exe printf(sepcmd, 3)
  let menucmd = 'amenu <silent> 41.333 '.a:menu.'.%s<Tab>'.a:leader.'%s %s'
  call s:addMenu(menucmd, 'MultiEncoding(&G)'           , 'rm', ':<C-u>call qfixlist#ToggleMultiEncodingGrep()<CR>')
  call s:addMenu(menucmd, 'SetFileEncoding(&G)'         , 'rG', ':<C-u>call <SID>SetFileEncoding()<CR>')
  call s:addMenu(menucmd, 'CurrentDirMode(&D)'          , 'rD', ':<C-u>call <SID>ToggleGrepCurrentDirMode()<CR>')
  call s:addMenu(menucmd, 'RecursiveMode(&M)'           , 'rM', ':<C-u>call <SID>ToggleGrepRecursiveMode()<CR>')
  " exe printf(sepcmd, 4)
...
if g:MyGrep_Keymap
  ...
  exe 'silent! nnoremap <unique> <silent> '.s:MyGrep_Key.'rD  :<C-u>call <SID>ToggleGrepCurrentDirMode()<CR>'
  exe 'silent! nnoremap <unique> <silent> '.s:MyGrep_Key.'rM  :<C-u>call <SID>ToggleGrepRecursiveMode()<CR>'
  exe 'silent! nnoremap <unique> <silent> '.s:MyGrep_Key.'rm  :<C-u>call qfixlist#ToggleMultiEncodingGrep()<CR>'
