...
let s:html_number_lines   = 0
let s:html_no_pre         = 1
let s:html_ignore_folding = 1
let s:html_whole_filler   = 1

if exists("s:html_number_lines")
  let s:numblines = s:html_number_lines
else
...
" Cache s:html_no_pre incase we have to turn it on for non-css mode
if exists("s:html_no_pre")
  let s:old_html_no_pre = s:html_no_pre
endif
let s:old_html_no_pre = s:html_no_pre
...
if has('folding') && !exists('s:html_ignore_folding')
...
while s:lnum <= s:end
  ...
  if s:filler > 0
	...
	let s:new = repeat(s:difffillchar, 3)
      ...
      if s:n > 2 && s:n < s:filler && !exists("s:html_whole_filler")
...
"----modify
    ...
    let s:new = repeat(' ', strlen(s:end) - strlen(s:lnum)) . s:lnum-html_start_line+1 . ': '
  else
    let s:new = ""
  endif

  if has('folding') && !exists('s:html_ignore_folding') && foldclosed(s:lnum) > -1
    "
	...
	if s:len < &columns && !exists("s:html_no_pre")
...
if exists("s:old_html_no_pre")
  let s:html_no_pre = s:old_html_no_pre
  unlet s:old_html_no_pre
elseif exists("s:html_no_pre")
  unlet s:html_no_pre
endif
